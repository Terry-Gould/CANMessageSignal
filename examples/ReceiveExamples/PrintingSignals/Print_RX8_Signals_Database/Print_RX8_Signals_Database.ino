...
void setup() {
  ...
  Serial.println("CANMessageSignal 1.1.1 RX database signal live-view example");
...
static void decodeWithDatabase(const CANFDMessage& rx) {
  if (!rxDb.decode(rx) && rxDb.hasError()) {
...
void loop() {
  ...
  while (can0.receiveFD0(FD0rx)) {
    decodeWithDatabase(FD0rx);
  }

  while (can0.receiveFD1(FD1rx)) {
    decodeWithDatabase(FD1rx);
  }
